 ...
 * 
 */
public class CopyOfTestStockToHtmls {
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			...
			if (aa.length > 1) {
				if (Util.notNull(aa[0]))
					filename = aa[0];
				if (Util.notNull(aa[1]))
					dirName = aa[1];
			} else {
				System.out.println("输入的参数格式是‘源文件名;目录名’");
				return;
			}
		}
		...
		for (int i = 0; i < strs.length; i++) {
			if (strs[i] != null && strs[i].length() > 0) {
				...
				new FileUtil.WriteFileBuilder(dirName + "\\"
						+ getStockName(strs[i]) + "_" + i + ".html").encoding("GBK").build()
						.writeFile(getHTMLByStockStr(stock));
			}
		}
		new FileUtil.WriteFileBuilder(dirName + "\\汇总.html").encoding("GBK").build().writeFile(
				getSumStockHtml(stocks));
			...
			while (it.hasNext()) {
				...
				if (stock.getStockName().indexOf("_") != -1) {
					...
					buf.append("<a href='").append(stock.getStockName())
							...
							.append("</a>").append("<br>").append(stock.getAllBuyCost()).append("</td>");
	...
	private static NewStock getStockByStr(String operStr, double endValue,
		...
		String[][] stocks = CopyOfTestStockToHtmls.splitStrsWith2Chars(operStr, ",",
				";");
